      ...
      create or replace transient table PROD_ENTDWDB.fds_yt.agg_yt_monetization_detail_dbt  as
      (
...
(select a.*,d.* 
from  
(select * from   PROD_ENTDWDB.fds_yt.rpt_yt_wwe_engagement_daily  where report_date between 20190101 and cast(to_char(current_date,'YYYYMMDD') as int)) a 
left join
(select distinct upper(iso_alpha2_ctry_cd) as iso_alpha2_ctry_cd ,initcap(country_nm) as country_name,region_nm from PROD_ENTDWDB.cdm.dim_region_country 
...
from 
(select a.*,d.* from (select * from   PROD_ENTDWDB.fds_yt.rpt_yt_revenue_daily where report_date between 20190101 and cast(to_char(current_date,'YYYYMMDD') as int)) a 
left join
(select distinct upper(iso_alpha2_ctry_cd) as iso_alpha2_ctry_cd ,initcap(country_nm) as country_name,region_nm from PROD_ENTDWDB.cdm.dim_region_country 
...
left join (select distinct report_date_dt debut_date, video_id from  PROD_ENTDWDB.fds_yt.rpt_yt_wwe_engagement_daily where video_uploaded_flg=1 ) c
