...
with __dbt__CTE__intm_yt_daily_wwe_video_pastyear as (
...
where date(time_uploaded) > date(getdate()-365)  
...
),  __dbt__CTE__intm_yt_daily_wwe_talent_pastyear as (


with cte as 
(select n::int from
  (select row_number() over (order by true) as n from __dbt__CTE__intm_yt_daily_wwe_video_pastyear)
cross join
(select  max(regexp_count(talent, '[,]')) as max_num from __dbt__CTE__intm_yt_daily_wwe_video_pastyear) where n <= max_num + 1)
select distinct time_uploaded, trim(SPLIT_PART(talent,',',n)) as talent, video_id,
 region_code, ttl_views,views_30days
 from __dbt__CTE__intm_yt_daily_wwe_video_pastyear
cross join cte
...
from __dbt__CTE__intm_yt_daily_wwe_talent_pastyear a
