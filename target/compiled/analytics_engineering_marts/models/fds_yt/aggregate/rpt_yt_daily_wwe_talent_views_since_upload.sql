...
with __dbt__CTE__intm_yt_daily_wwe_video_since_upload as (
...
where date(time_uploaded) > '2017-01-01'  
...
),  __dbt__CTE__intm_yt_daily_wwe_talent_since_upload as (


with cte as 
(select n::int from
  (select row_number() over (order by true) as n from __dbt__CTE__intm_yt_daily_wwe_video_since_upload)
cross join
(select  max(regexp_count(talent, '[,]')) as max_num from __dbt__CTE__intm_yt_daily_wwe_video_since_upload) where n <= max_num + 1)
select distinct time_uploaded, trim(SPLIT_PART(talent,',',n)) as talent, video_id,
 region_code, ttl_views,views_30days
 from __dbt__CTE__intm_yt_daily_wwe_video_since_upload
cross join cte
where  split_part(talent,',',n) is not null and split_part(talent,',',n) != ''
)select region_code,initcap(country_nm) as region_name, talent, 'since upload' as granularity,
...
from __dbt__CTE__intm_yt_daily_wwe_talent_since_upload a
