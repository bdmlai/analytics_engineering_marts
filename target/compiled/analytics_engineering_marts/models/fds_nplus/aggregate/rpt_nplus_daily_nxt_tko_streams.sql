...
with __dbt__CTE__intm_nplus_content_nxt_tko as (
...
),  __dbt__CTE__intm_nplus_duration_nxt_tko as (
...
),  __dbt__CTE__intm_nplus_litelog_est_time_diff_nxt_tko as (
...
where lower(title) like '%nxt: takeover%' and issegmentmarker='TRUE'
 and upper(segmenttype) in ('MATCH','BACKSTAGE ON CAMERA','IN-ARENA NON-MATCH','ON CAMERA','TALK SHOW') 
 and airdate in ( select premiere_date from __dbt__CTE__intm_nplus_content_nxt_tko) 
...
),  __dbt__CTE__intm_nplus_lite_log_est_nxt_tko as (
...
and lower(title) like '%nxt: takeover%' and issegmentmarker='TRUE'
 and upper(segmenttype) in ('MATCH','BACKSTAGE ON CAMERA','IN-ARENA NON-MATCH','ON CAMERA','TALK SHOW')
),  __dbt__CTE__intm_nplus_event_litelog_nxt_tko as (
...
),  __dbt__CTE__intm_nplus_viewership_sequence_generator_nxt_tko as (
...
(select count(distinct stream_id) from __dbt__CTE__intm_nplus_viewershipdata_with_externalid_nxt_tko b where a.external_id = b.external_id and b.min_time < a.time_interval
) as streams_count,
(select count(distinct src_fan_id) from __dbt__CTE__intm_nplus_viewershipdata_with_externalid_nxt_tko c where a.external_id = c.external_id and (c.min_time < a.time_interval) 
) as cumulative_unique_user,
(select count(distinct src_fan_id) from __dbt__CTE__intm_nplus_viewershipdata_with_externalid_nxt_tko  where a.external_id = external_id and min_time < a.time_interval 
and  min_time > a.prev_time_interval
) as users_added,
(select count(distinct src_fan_id) from __dbt__CTE__intm_nplus_viewershipdata_with_externalid_nxt_tko  where a.external_id = external_id and max_time < a.time_interval and max_time > a.prev_time_interval  
) as users_exits,	
(select count(distinct src_fan_id) from __dbt__CTE__intm_nplus_viewershipdata_with_externalid_nxt_tko  where a.external_id = external_id and min_time < a.time_interval and max_time > a.time_interval  
) as total_user,
(select count(distinct src_fan_id) from __dbt__CTE__intm_nplus_viewershipdata_with_externalid_nxt_tko  where a.external_id = external_id and min_time < a.prev_time_interval and max_time > a.time_interval  
...
from __dbt__CTE__intm_nplus_viewership_sequence_generator_nxt_tko a