...
from 
(select distinct  cast('shop' as varchar(10)) as business_unit, cast(cast(a.ship_date_id as varchar(10)) as date) as date, a.dim_shop_site_id,
...
from 
        ...
        ship_date_id, src_order_number, src_order_origin_code, src_order_status, src_order_type, src_original_ref_order_number,
		src_pre_order_flag, src_prepay_code, src_promo_code, src_return_reason_code, src_sequence_number, src_ship_carrier, 
		src_shipped_flag, dim_shop_site_id, dim_source_sys_id, src_special_instructions, src_unit_cost, src_unit_cost_local, 
        ...
        from 
                (select distinct a.*, b.src_style_id, b.src_style_description, b.src_category_description,
                 ...
                 from 
                        ...
                        from 
                        (select * from "entdwdb"."fds_cpg"."fact_cpg_sales_detail" where  dim_shop_site_id <= 11 and
						...
						left join "entdwdb"."fds_cpg"."dim_cpg_shop_site" d on a.dim_shop_site_id = d.dim_shop_site_id
                        left join "entdwdb"."fds_cpg"."dim_cpg_item" c on a.dim_item_id = c.dim_item_id
                        left join "entdwdb"."fds_cpg"."fact_cpg_inventory" b on a.dim_item_id = b.dim_item_id and a.ship_date_id = b.dim_date_id 
                ...
                left join (select * from (select *, row_number() over(partition by src_item_id order by effective_start_datetime desc) as rank 
										  from "entdwdb"."fds_cpg"."dim_cpg_item" ) where rank = 1) b 
        ...
        from 
                (select distinct a.*, b.src_style_id, b.src_style_description, b.src_category_description,
                 ...
                 from 
                        ...
                        from "entdwdb"."fds_cpg"."fact_cpg_sales_detail_kit_component" a
						left join "entdwdb"."fds_cpg"."dim_cpg_shop_site" b on a.dim_shop_site_id = b.dim_shop_site_id
                        left join "entdwdb"."fds_cpg"."dim_cpg_item" c on a.dim_item_id = c.dim_item_id) a 
                left join (select * from (select *, row_number() over(partition by src_item_id order by effective_start_datetime desc) as rank 
										  from "entdwdb"."fds_cpg"."dim_cpg_item" ) where rank = 1) b 
                on a.src_item_id = b.src_item_id)) a 
group by 1,2,3,4,5,6,7,8,9) 
...
from 
        ...
        from
               (select distinct a.dim_agg_sales_id, a.date_id, a.dim_item_id, a.dim_event_id, a.dim_venue_id, a.quantity_shipped,
                ...
                from "entdwdb"."fds_cpg"."aggr_cpg_daily_venue_sales" a left join "entdwdb"."fds_cpg"."dim_cpg_item" b on a.dim_item_id = b.dim_item_id
...
group by 1,2,3,4,5,6,7,8,9)) 
where date >= dateadd('day', -1500, current_date)