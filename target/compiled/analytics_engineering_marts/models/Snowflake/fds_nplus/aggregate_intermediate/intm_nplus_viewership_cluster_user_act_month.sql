...
renamed as (
    ...
    select 
        ...
        ifnull(nullif('None','None'),date_trunc(month,current_date)-1) as as_on_date,   --convert to string so that script can be replaced by another date using python wrapper
...
aggregated as (
    ...
    select 
        ...
        datediff(mon,'2015-01-01',ifnull(nullif('None','None'),date_trunc(month,current_date)-1)) as max_months_available
    from active_months
    group by 1  
...
cleaned as (
    ...
    ifnull(nullif('None','None'),date_trunc(month,current_date)-1) as as_on_date
