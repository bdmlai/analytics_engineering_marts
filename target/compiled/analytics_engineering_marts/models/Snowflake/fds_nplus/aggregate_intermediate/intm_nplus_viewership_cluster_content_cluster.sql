...
base as (
    ...
    select * from "prod_entdwdb"."cdm"."dim_content_classification_title"
    
),

daily_postshow_rankings as (
    
    select * from "prod_entdwdb"."fds_nplus"."aggr_daily_postshow_rankings"
    
),

daily_content_viewership as (
    
    select * from "prod_entdwdb"."fds_nplus"."fact_daily_content_viewership"
    where stream_start_dttm between '2015-01-01' and ifnull(nullif('None','None'),date_trunc(month,current_date)-1)
    ...
    select 
      ...
      -- regroup content_class into high level categories
       case when content_class in ('Tier 1 PPV','Tier 2 PPV','NXT TakeOver') then 'Big Event'
        ...
        to_date(to_date(to_char(ifnull(nullif('None','None'),date_trunc(month,current_date)-1)),'yyyy-mm-dd')) as as_on_date,
