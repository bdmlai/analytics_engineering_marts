...
base as (
    ...
    select * from "prod_entdwdb"."fds_nplus"."fact_daily_subscription_status_plus"
    where as_on_date in (select max(as_on_date) from "prod_entdwdb"."fds_nplus"."fact_daily_subscription_status_plus")
),

cluster as (
    
    select * from "entdwdb"."dt_stage"."intm_nplus_viewership_cluster_content_cluster"
...
orders as (
    ...
    select 
        ...
        to_date(to_date(to_char(ifnull(nullif('None','None'),date_trunc(month,current_date)-1)),'yyyy-mm-dd')) as as_on_date,   --convert to string so that script can be replaced by another date using python wrapper
...
with_id as (
    ...
    coalesce(cast(src_fan_id as varchar), '') || '-' || coalesce(cast(production_id as varchar), '') || '-' || coalesce(cast(order_id as varchar), '')
