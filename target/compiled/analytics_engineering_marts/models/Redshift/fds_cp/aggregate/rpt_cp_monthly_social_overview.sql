...
from 	"entdwdb"."fds_fbk"."vw_rpt_daily_fb_published_video"
...
from 	"entdwdb"."fds_fbk"."vw_rpt_daily_fb_published_post"
...
from 	"entdwdb"."fds_tw"."vw_rpt_daily_tw_published_post"
where 	month = date_trunc('month',current_date - 28)
group by 1,2,3,4),
...
(select date_trunc('month',post_date) as month,
		...
		dim_smprovider_account_id,
		(count (distinct dim_video_id) + count (distinct dim_media_id)) as Value
from 	"entdwdb"."fds_igm"."vw_rpt_daily_ig_published_post"
where 	month = date_trunc('month',current_date - 28)
group by 1,2,3
...
select 	date_trunc('month',frame_date) as month,
		...
		dim_smprovider_account_id,
		(count (distinct dim_video_id) + count (distinct dim_media_id)) as Value
from  "entdwdb"."fds_igm"."vw_rpt_daily_ig_published_frame"
where 	month = date_trunc('month',current_date - 28)
group by 1,2,3)
group by 1,2,3,4,5),
...
(select  date_trunc('month',to_date(dim_date_id,'yyyymmdd')) as month,
		...
		dim_smprovider_account_id,
		sum(video_views) as Value 
from "entdwdb"."fds_igm"."fact_ig_consumption_post"
where month = date_trunc('month',current_date-28)
group by 1,2,3
...
from 	"entdwdb"."fds_igm"."fact_ig_consumption_frame"
where 	month = date_trunc('month',current_date - 28)
group by 1,2,3)
group by 1,2,3,4,5
),
...
from "entdwdb"."fds_fbk"."vw_rpt_daily_fb_published_video" a
...
from "entdwdb"."fds_fbk"."fact_fb_engagement_post"
where month = date_trunc('month',current_date-28)
and dim_content_type_id not in ('10236','10003','10230','10234','10257','10260')
group by 1,2,3
)
...
from "entdwdb"."fds_igm"."fact_ig_engagement_post"
where month = date_trunc('month',current_date-28)
group by 1,2,3
...
from "entdwdb"."fds_igm"."fact_ig_engagement_frame"
where month = date_trunc('month',current_date-28)
group by 1,2,3
)
group by 1,2,3,4),
...
left join 
(select * from "entdwdb"."hive_udl_cp"."daily_social_account_country_mapping"
...
left join "entdwdb"."fds_cp"."rpt_cp_monthly_social_overview" b
