...
prev_year_followers
from (select as_on_date, platform, account, designation, brand, gender, sum(followers) as followers from "entdwdb"."fds_cp"."rpt_cp_weekly_social_followership"
group by 1,2,3,4,5,6) a
left join 
(select as_on_date, platform, account, sum(followers) as prev_week_followers from "entdwdb"."fds_cp"."rpt_cp_weekly_social_followership"
group by 1,2,3) b
on a.as_on_date-7 = b.as_on_date and a.platform = b.platform 
and a.account = b.account
left join 
(select as_on_date, platform, account, sum(followers) as prev_month_followers from "entdwdb"."fds_cp"."rpt_cp_weekly_social_followership"
group by 1,2,3) c
