...
from(
(select as_on_date,count(distinct account) as total_accounts,sum(followers) as followers from "entdwdb"."fds_cp"."rpt_cp_weekly_social_followership"
group by 1) a
left join 
(select as_on_date,count(distinct account) as total_accounts_last_week,sum(followers) as prev_week_followers from "entdwdb"."fds_cp"."rpt_cp_weekly_social_followership"
group by 1)b
on a.as_on_date-7=b.as_on_date
left join 
(select as_on_date,count(distinct account) as total_accounts_last_month,sum(followers) as prev_month_followers from "entdwdb"."fds_cp"."rpt_cp_weekly_social_followership"
group by 1
)c
