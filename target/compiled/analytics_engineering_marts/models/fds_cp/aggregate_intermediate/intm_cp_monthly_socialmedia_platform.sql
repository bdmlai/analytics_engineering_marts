...
sum(c_followers)as followers from "entdwdb"."fds_fbk"."fact_fb_smfollowership_audience_bycountry" group by 1,2))
where followers is not null group by 1,2 order by 1,2 desc) as a
left join 

/*Instagram Followers */--ephemeral
(select * from __dbt__CTE__intm_instagram_followers) as b
on a.date=b.date and 
a.dim_country_id=b.dim_country_id

left join --ephemeral country_population
(select * from __dbt__CTE__intm_country_population) as c
on a.dim_country_id = c.dim_country_id

left join

/*YouTube Followers */--ephemearl
(select * from __dbt__CTE__intm_youtube_followers) as d 
