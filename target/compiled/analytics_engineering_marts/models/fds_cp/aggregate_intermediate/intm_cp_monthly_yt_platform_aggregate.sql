...
select trunc(date_trunc('month',report_date)) as month,
       ...
       -- abs(round(sum(coalesce(ugc_views,0)))) as values ,
        'YouTube' as platform      
 from   __dbt__CTE__intm_cp_monthly_yt_platform
 ...
 select trunc(date_trunc('month',report_date)) as month,
       ...
       -- abs(round(sum(coalesce(owned_views,0)))) as values ,
        'YouTube' as platform       
 from   __dbt__CTE__intm_cp_monthly_yt_platform
 group by month,country,metric
 ...
 select trunc(date_trunc('month',report_date)) as month,
       ...
       -- round(sum(ugc_hours_watched)) as values,
       -- abs(round(sum(coalesce(ugc_hours_watched,0)))) as values,
        'YouTube' as platform 
 from   __dbt__CTE__intm_cp_monthly_yt_platform
 group by month,country,metric
 ...
 select trunc(date_trunc('month',report_date)) as month,
        ...
        --abs(round(sum(coalesce(owned_hours_watched,0)))) as values,
        'YouTube' as platform       
 from   __dbt__CTE__intm_cp_monthly_yt_platform
 group by month,country,metric
