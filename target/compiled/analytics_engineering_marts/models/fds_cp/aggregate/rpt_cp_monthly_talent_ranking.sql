...
from fds_yt.rpt_yt_wwe_engagement_daily wed where date_trunc('month',report_date_dt)= date_trunc('month',current_date-28)
group by 1,2,3
) k
left join fds_yt.yt_amg_content_groups acg on (k.video_id = acg.yt_id)),
...
from fds_fbk.vw_rpt_daily_fb_published_video a
...
from fds_fbk.fact_fb_engagement_post
where month = date_trunc('month',current_date-28)
...
from fds_cp.fact_co_smfollowership_cumulative_summary 
...
from fds_tw.fact_tw_engagement_post
where month = date_trunc('month',current_date-28)
group by 1,2,3,4),
...
left join cdm.dim_smprovider_account c
...
from  ( select distinct src_adj_txn_flag,src_back_order_flag,dim_business_unit_id,dim_customer_email_address_id,dim_order_method_id,src_current_retail_price,src_current_retail_price_local,
 ...
 from  (select * from fds_cpg.fact_cpg_sales_detail where dim_shop_site_id<=6 and
 (order_date_id>=0 or ship_date_id>=0) and 
 isnull(dim_order_method_id,0) NOT IN (5) and 
 dim_item_id not in (
 select distinct B.dim_item_id from (select A.*,B.src_item_id from fds_cpg.dim_cpg_kit_item(nolock) A inner join fds_cpg.dim_cpg_item(nolock) B on A.dim_kit_item_id=B.dim_item_id) A, fds_cpg.dim_cpg_item B where A.src_item_id=B.src_item_id )
 and src_order_number not in (
 SELECT distinct ltrim(rtrim(src_order_number)) As src_order_number FROM fds_cpg.fact_cpg_sales_header where ltrim(rtrim(src_order_origin_code))='GR' or ltrim(rtrim(src_prepay_code))='F') 
 and src_order_number in (
 SELECT distinct ltrim(rtrim(src_order_number)) As src_order_number FROM fds_cpg.fact_cpg_sales_header(nolock) where ltrim(rtrim(src_original_ref_order_number))='0') 
 ) a 
 left join fds_cpg.dim_cpg_item c on a.dim_item_id=c.dim_item_id
 left join fds_cpg.fact_cpg_inventory b on a.dim_item_id=b.dim_item_id and a.order_date_id=b.dim_date_id and a.dim_business_unit_id=b.dim_business_unit_id and a.dim_shop_site_id=b.dim_shop_site_id) a 
 left join (select * from (select *,row_number() over(partition by src_item_id order by effective_start_datetime desc) as rank from fds_cpg.dim_cpg_item ) where rank=1) b 
...
from 
 ...
 when src_wwe_order_number like '900%' then 10 else dim_shop_site_id end as dim_shop_site_id_new
 FROM 
 (select distinct a.*,c.src_item_id from (select * from fds_cpg.fact_cpg_sales_detail_kit_component ) a left join fds_cpg.dim_cpg_item c on a.dim_item_id=c.dim_item_id) a 
 left join (select * from (select *,row_number() over(partition by src_item_id order by effective_start_datetime desc) as rank from fds_cpg.dim_cpg_item ) where rank=1) b 
 on a.src_item_id=b.src_item_id
 )
 ) a 
...
from 
 ...
 from fds_cpg.aggr_cpg_daily_venue_sales a left join fds_cpg.dim_cpg_item b on a.dim_item_id=b.dim_item_id) a
 left join (select * from (select *,row_number() over(partition by src_item_id order by effective_start_datetime desc) as rank from fds_cpg.dim_cpg_item ) where rank=1) b 
 on a.src_item_id=b.src_item_id ) 
group by 1,2,3,4,5,6,7,8,9 )
---
