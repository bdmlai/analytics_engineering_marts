...
where report_date>20180000 and report_date<=(select floor(max(report_date)/100)*100 from "entdwdb"."fds_yt"."rpt_yt_wwe_engagement_daily")
group by 1,2,3,4,5
union
--ugc content
select country_code,  to_char(trunc(convert(datetime,convert(varchar(10),report_date))),'YYYYMM') as year_month, 
...
case when datediff('day',time_uploaded,report_date_dt)<=30 then 'Recent' else 'Old' end as recency,
sum(views) as views, sum(subscribers_gained-subscribers_lost) as netsubs, sum(watch_time_minutes)/60 as hours_watched 
from "entdwdb"."fds_yt"."rpt_yt_ugc_engagement_daily" where video_id not in 
(select distinct video_id from "entdwdb"."fds_yt"."rpt_yt_wwe_engagement_daily") and report_date>20180000 and
 report_date<=(select floor(max(report_date)/100)*100
from "entdwdb"."fds_yt"."rpt_yt_ugc_engagement_daily" ) group by 1,2,3,4,5
) a 
left join
--revenue
(select country_code,  to_char(trunc(convert(datetime,convert(varchar(10),report_date))),'YYYYMM') as year_month, 
case when channel_name='WWE' then 'WWE' else 'Other' end as channel_name, 
case when video_id in (select distinct video_id from "entdwdb"."fds_yt"."rpt_yt_wwe_engagement_daily") then
 'WWE' else 'UGC' end as content_type, 
case when video_id in (select video_id from "entdwdb"."fds_yt"."rpt_yt_wwe_engagement_daily" where 
datediff('day',time_uploaded,report_date_dt)<=30) then 'Recent' when video_id in 
(select video_id from "entdwdb"."fds_yt"."rpt_yt_ugc_engagement_daily" where datediff('day',time_uploaded,report_date_dt)<=30) 
then 'Recent' else 'Old' end as recency,
sum(estimated_partner_revenue) as revenue, sum(gross_revenue) as gross_revenue from "entdwdb"."fds_yt"."rpt_yt_revenue_daily" 
...
a.recency=b.recency
left join 
(select distinct iso_alpha2_ctry_cd, country_nm, region_nm as region from "entdwdb"."cdm"."dim_region_country"
