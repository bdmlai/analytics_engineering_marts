...
(select distinct video_id+report_date from  {{source('fds_yt','rpt_yt_revenue_daily')}} where estimated_partner_revenue!=0 group by video_id, report_date)
...
from {{source('fds_yt','rpt_yt_revenue_daily')}} where report_date between to_char(current_date - 52, 'YYYYMMDD')   
...
left join 
(select distinct video_id, channel_name from  {{source('fds_yt','youtube_video_metadata_direct')}} 
 where as_on_date=(select max(as_on_date) from {{source('fds_yt','youtube_video_metadata_direct')}})) m
