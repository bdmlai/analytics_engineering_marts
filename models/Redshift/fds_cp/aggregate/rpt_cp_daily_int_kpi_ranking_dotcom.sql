{{
  config({
		'schema': 'fds_cp',
		"pre-hook": "truncate fds_cp.rpt_cp_daily_int_kpi_ranking_dotcom",
...
left join
(select distinct country_nm, region_nm from {{source('cdm','dim_region_country')}} where ent_map_nm = 'GM Region') f
