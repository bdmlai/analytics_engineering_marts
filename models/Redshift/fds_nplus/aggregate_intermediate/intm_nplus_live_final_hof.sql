        ...
        CASE WHEN a.event_date                         =
                        (
                                SELECT
                                        MAX(event_date)
                                FROM
                                        {{ ref("intm_nplus_live_consolidation_hof") }}) THEN 'Most Recent PPV' ELSE 'Prior PPVs' END AS Choose_PPV
FROM
        {{ ref("intm_nplus_live_consolidation_hof") }} a
LEFT JOIN
        (
                SELECT
                        platform  ,
                ...
                FROM
                        {{ ref("intm_nplus_live_consolidation_hof") }}) AS b
ON
