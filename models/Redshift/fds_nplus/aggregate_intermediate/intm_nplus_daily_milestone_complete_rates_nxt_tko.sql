...
from
(
select distinct 'TakeOver' as type,  external_id, title, airdate::varchar as premiere_date, 
...
(select distinct *, case when viewed_mins>content_duration then 1.00 else viewed_mins/content_duration end as complete_rate from
                ...
                from (select * from {{ref('intm_nplus_viewershipdata_with_externalid_nxt_tko')}} 
                 where  min_time between live_start and live_end) group by 1,2,3) a
                left join  {{ref('intm_nplus_event_litelog_nxt_tko')}} b 
