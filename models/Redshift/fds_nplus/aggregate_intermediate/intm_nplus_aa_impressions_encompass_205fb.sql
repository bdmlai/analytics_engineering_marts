...
SELECT CAST(CASE
    ...
    CAST('LITE' AS varchar) || CAST(logentrydbid AS varchar) AS content_id, comment AS content_description, duration AS duration, 
    CAST(CASE
    WHEN title= 'Live Event (Saudi Arabia Royal Rumble)' THEN
    ...
    CAST('Litelog' AS varchar) AS Ad_Abbreviation, CAST('Litelog ' AS varchar) || CAST(segmenttype AS varchar) AS ad_category, CAST('Litelog' AS varchar) AS ad_type
