...
SELECT
        ...
        CASE WHEN a.event_brand                        = 'NXT' AND     nxt_yearly_rank>0 THEN nxt_yearly_rank ELSE NULL END                                                                                                                                                                                                     AS yearly_rank  ,
        ...
        CASE WHEN a.event_date                         =
                        (
                                SELECT
                                        MAX(event_date)
                                FROM
                                        {{ ref("intm_nplus_live_plus_vod_consolidation_nxt") }}) THEN 'Most Recent PPV' ELSE 'Prior PPVs' END AS Choose_PPV
FROM
        {{ ref("intm_nplus_live_plus_vod_consolidation_nxt") }} a
LEFT JOIN
        (
                SELECT
                        platform  ,
                        ...
                        (row_number() OVER (PARTITION BY
                                            platform ORDER BY
                                            views DESC)) AS nxt_yearly_rank
                FROM
                        {{ ref("intm_nplus_live_plus_vod_consolidation_nxt") }}
                WHERE
                        TRUNC(convert_timezone('AMERICA/NEW_YORK', SYSDATE))-event_date::date <=735
                AND     event_brand                                                            = 'NXT') AS b
ON
