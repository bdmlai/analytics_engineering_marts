...
LEFT JOIN 
    (SELECT filter_name,
        concurrent_plays,
        ts2,
         lead(concurrent_plays) OVER (partition by filter_name ORDER BY  ts2) AS plays_upp, 
    ...
    FROM 
        ...
        FROM 
            ...
            rank() over( partition by pulse_timestamp,filter_name ORDER BY  as_on_date DESC ) AS rank
            FROM {{source('hive_udl_ads','conviva_daily_pulse')}}
            WHERE pulse_timestamp::date >= '2017-01-01' )
            WHERE rank = 1 ) ) t2
            ON t1.end_time> t2.ts2 AND t1.end_time<=t2.ts2_upp 