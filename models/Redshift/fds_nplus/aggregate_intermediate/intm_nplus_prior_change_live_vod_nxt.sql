...
FROM
        (
                SELECT
                        ...
                        event_brand
                FROM
                        {{source('fds_nplus','rpt_nplus_monthly_nxt_live_vod')}}
                WHERE
                        data_level = 'Live+VOD'
                AND     event_brand IN
                        (
                                SELECT DISTINCT
                                        event_brand
                                FROM
                                        {{ ref("intm_nplus_live_plus_vod_manual_nxt") }})
                AND     event IN
                        (
                                SELECT DISTINCT
                                        prev_month_event
                                FROM
                                        {{ ref("intm_nplus_live_plus_vod_manual_nxt") }})) AS a
JOIN
        (
                SELECT
                        platform                ,
                ...
                AND     event IN
                        (
                                SELECT DISTINCT
                                        prev_year_event
                                FROM
                                        {{ ref("intm_nplus_live_plus_vod_manual_nxt") }})) AS b
ON
