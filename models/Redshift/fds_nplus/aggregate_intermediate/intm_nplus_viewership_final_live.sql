...
select *,
(select count(distinct stream_id) from {{ref('intm_nplus_viewershipdata_with_externalid_live')}} b 
where a.external_id = b.external_id and b.min_time < a.time_interval and b.max_time > a.time_interval
) as streams_count,
(select count(distinct src_fan_id) from {{ref('intm_nplus_viewershipdata_with_externalid_live')}} c 
where a.external_id = c.external_id and (c.min_time <= a.time_interval) 
...
(select count(distinct src_fan_id) from {{ref('intm_nplus_viewershipdata_with_externalid_live')}}  
