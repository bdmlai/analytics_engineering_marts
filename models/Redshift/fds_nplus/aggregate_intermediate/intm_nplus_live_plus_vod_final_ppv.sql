        ...
        CASE WHEN a.event_date                         =
                        (
                                SELECT
                                        MAX(event_date)
                                FROM
                                        {{ ref("intm_nplus_live_plus_vod_consolidation_ppv") }}) THEN 'Most Recent PPV' ELSE 'Prior PPVs' END AS Choose_PPV
FROM
        {{ ref("intm_nplus_live_plus_vod_consolidation_ppv") }} a
LEFT JOIN
        (
                SELECT
                        platform  ,
                ...
                FROM
                        {{ ref("intm_nplus_live_plus_vod_consolidation_ppv") }}
                WHERE
