...
SELECT substring(CAST(strt_dt AS varchar) || ' ' || CAST(strt_tm AS varchar), 1, 19) AS start_time_join, 
    CAST(strt_dt AS varchar) AS Start_Date,
    substring(strt_tm, 1, 8) AS Start_Time,
    upper(cntnd_id) Content_ID,
    CASE
    ...
    CASE
    WHEN substring(cntnd_id, 1,4)= 'wwad' OR substring(cntnd_id, 1,4)='wwbb' THEN 'Billboards'
    ...
    END AS ad_category,
    CASE
    WHEN cntnd_id LIKE '%wwb%'  OR cntnd_id LIKE '%wwad%' THEN 'Client Advertising'
    WHEN cntnt_desc LIKE '%komen%'
        ...
        OR cntnt_desc LIKE '%connor%' THEN 'Community Relations (CSR)'
    ...
    WHEN (cntnd_id LIKE '%wwid%'
        ...
        OR cntnd_id LIKE '%wwop%') THEN 'WWE Promos'
    ELSE 'Other'
    END AS Ad_Type
