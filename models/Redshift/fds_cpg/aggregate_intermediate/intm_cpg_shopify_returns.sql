...
SELECT
    ...
    TRUNC(date_trunc('month', to_date(COALESCE(sd.ship_date_id,sd.order_date_id), 'YYYYMMDD'))) AS
                                                         report_month,
    (COALESCE(SUM(ABS(sd.src_units_shipped)),0) * -1)           AS returned_quantity,
    (COALESCE(SUM(ABS(sd.src_selling_price_local)),0.000) * -1) AS returned_sales
FROM
