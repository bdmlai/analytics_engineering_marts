...
FROM
    (
        ...
        AND MONTH = TRUNC(date_trunc('month',add_months(CURRENT_DATE,-1)))
        GROUP BY
...
LEFT JOIN
    (
        SELECT
            *,
            row_number() over (ORDER BY revenue DESC) AS rank_last_month
        FROM
            (
                SELECT
                    TRUNC(date_trunc('month',CAST(CAST(s.date_key AS VARCHAR(10)) AS DATE))) AS
                    MONTH,
                    CASE
                        WHEN i.src_talent_description LIKE '%ALMAS%'
                        THEN 'ANDRADE ''CIEN'' ALMAS'
                ...
                FROM
                    ...
                    {{source('fds_cpg','dim_cpg_item')}} i
                WHERE
                    MONTH = TRUNC(date_trunc('month',add_months(CURRENT_DATE,-2)))
