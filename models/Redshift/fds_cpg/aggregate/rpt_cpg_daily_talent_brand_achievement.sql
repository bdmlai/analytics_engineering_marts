...
from 
	(select distinct i.src_talent_description as talent_description, a.entity_id, a.entity_type, b.brand
        from {{source('fds_cpg','dim_cpg_item')}} i
        join {{source('fds_mdm','alternateid')}} a on a.alternate_id_name = i.src_talent_id 
        ...
        left join (select distinct character_lineage_id as entity_id, a.character_lineages_wweid, 
					...
					from {{source('fds_mdm','character')}} a 
					left join {{source('fds_emm','brand')}} b on a.character_lineages_wweid = b.character_lineage_wweid 
     ...
     select distinct i.src_talent_description as talent_description, a.entity_id, a.entity_type, b.brand
        ...
        left join
                ...
                (select distinct group_lineage_id as entity_id, group_lineage_wweid, group_lineage_name, 
                  ...
                  from {{source('fds_mdm','characterlineage_grouplineage')}} a 
				  left join {{source('fds_emm','brand')}} b on a.character_lineage_wweid = b.character_lineage_wweid 
...
left join 
        ...
        from {{source('fds_mdm','achievement_entity')}} a 
		left join {{source('fds_mdm','achievement')}} b on a.achievement_id = b.id 
		left join {{source('fds_mdm','character')}} c on a.entity_id = c.id
        ...
        from {{source('fds_mdm','achievement_entity')}} a 
		left join {{source('fds_mdm','achievement')}} b on a.achievement_id = b.id 
		left join {{source('fds_mdm','groups')}} c on a.entity_id = c.mdmid
