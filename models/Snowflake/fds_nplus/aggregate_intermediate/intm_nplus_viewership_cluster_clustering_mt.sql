/***********************************************************************************/
/*                  7.  tier 3 model - rules based behaviors-final(mt)             */
/* db:analytics_workspace schema:content                                           */
/* input tables: intm_nplus_viewership_cluster_clustering_sub_rules_mt             */
/*               intm_nplus_viewership_cluster_user_act_month                      */
/* ouput tables: intm_nplus_viewership_cluster_clustering_mt                          */
/***********************************************************************************/
{{
    config(
        materialized='table',
...
base as (
    ...
    select * from {{ ref('intm_nplus_viewership_cluster_clustering_sub_rules_mt') }}
...
act_month as (
    ...
    select src_fan_id,
           mt_act_mo_clean as act_months
     from {{ ref('intm_nplus_viewership_cluster_user_act_month') }}
    where mt_act_mo_clean >= 0.5    
),
