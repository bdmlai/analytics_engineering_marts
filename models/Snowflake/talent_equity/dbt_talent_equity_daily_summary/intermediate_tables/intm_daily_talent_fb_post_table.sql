...
/*
*************************************************************************************************************************************************
   TableName   : intm_daily_talent_fb_post_table
   Schema	   : CREATIVE
   Contributor : Sourish Chakraborty
   Description : Intermediate Model to capture #Fb posts and engagements for talents
...
sum(tot_fb_post) as tot_fb_post,sum(tot_eng_fb_post) as tot_eng_fb_post
FROM ({{ref('base_daily_talent_table')}}) a 
LEFT JOIN 
(
({{ref('base_daily_talent_fb_post_table')}}))n 
