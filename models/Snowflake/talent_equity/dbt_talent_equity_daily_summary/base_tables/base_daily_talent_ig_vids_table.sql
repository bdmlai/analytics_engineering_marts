/*
*************************************************************************************************************************************************
   TableName   : base_daily_talent_ig_vids_table
   Schema	   : CREATIVE
   Contributor : Sourish Chakraborty
   Description : Base Model to capture #ig vids,views and engagements for talents
...
count(distinct dim_video_id) as tot_ig_vids,sum(engagements_total) as tot_eng_ig_vids,sum(minutes_viewed) as tot_mins_ig_vids,
sum(video_views) as tot_views_ig_vids
