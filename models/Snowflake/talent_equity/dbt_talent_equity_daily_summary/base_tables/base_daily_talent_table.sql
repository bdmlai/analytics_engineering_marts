...
(
select * from (
select full_date as date,character_lineage_name as lineage_name,character_lineages_wweid as lineage_wweid,source_string,gender,
case when entity_type='character_lineages' then 'Character' when entity_type='group_lineages' then 'Group' end as entity_type from(
...
character_lineage_id as entity_id,gender from {{source('sf_fds_mdm','character')}}
union all
select group_name,character_lineage_name,character_lineage_wweid,'group_lineages' as entity_type,
character_lineage_mdmid as entity_id,null as gender from {{source('sf_fds_mdm','groups')}})a
...
group by 1,2,3,4,5,6 order by 2,1) where lineage_name is not null
