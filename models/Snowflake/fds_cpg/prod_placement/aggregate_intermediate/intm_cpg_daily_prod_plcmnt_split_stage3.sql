/*
*************************************************************************************************************************************************
   TableName   : intm_cpg_daily_prod_plcmnt_split_stage3
   Schema	   : CREATIVE
...
FROM {{ref('intm_cpg_daily_prod_plcmnt_split_stage2')}}
CROSS JOIN
(SELECT cast(N as integer) as N FROM
(SELECT ROW_NUMBER() OVER (ORDER BY TRUE) AS N FROM {{ref('intm_cpg_daily_prod_plcmnt_split_stage2')}}))
