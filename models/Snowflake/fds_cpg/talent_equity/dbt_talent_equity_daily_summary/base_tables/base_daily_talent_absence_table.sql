/*
*************************************************************************************************************************************************
   TableName   : base_daily_talent_absence_table
   Schema	   : CREATIVE
   Contributor : Sourish Chakraborty
   Description : Base Model to capture absence of talents
...
{{ config(materialized = 'table',schema='dt_stage',
            enabled = true, 
                tags = ['talent equity','emm','absence','daily',
                        'centralized table'],
...
SELECT b.character_lineages_wweid, b.character_lineage_name, a.type as absence, a.start_date, 
CASE WHEN a.end_date IS NULL THEN CURRENT_DATE ELSE a.end_date END AS end_date 
FROM {{source('sf_fds_emm','absence')}} a LEFT JOIN {{source('sf_fds_mdm','character')}} b 
