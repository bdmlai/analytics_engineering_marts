...
drop table if exists user_act_month_0;
create temporary table user_act_month_0 as
select *,
       -- short-term
       case when start_date < prev_3_month
            then prev_3_month
            else start_date end as st_start,
       case when end_date >= today_date
            then dateadd(d,-1,today_date)
            else end_date end as st_end,
       -- mid-term
       case when start_date < prev_12_month 
            then prev_12_month
            else start_date end as mt_start,
       case when end_date >= prev_3_month
            then dateadd(d,-1,prev_3_month)
            else end_date end as mt_end,
       -- long-term
       case when start_date < '2015-01-01' 
            then '2015-01-01'
            else start_date end as lt_start,
       case when end_date >= prev_12_month
            then dateadd(d,-1,prev_12_month)
            else end_date end as lt_end,
       -- all_time
       case when start_date < '2015-01-01' 
            then '2015-01-01'
            else start_date end as all_time_start,
       case when end_date >= today_date
            then dateadd(d,-1,today_date)
            else end_date end as all_time_end,
       -- last_year
       case when start_date < prev_12_month
            then prev_12_month
            else start_date end as ly_start,
       case when end_date >= today_date
            then dateadd(d,-1,today_date)
            else end_date end as ly_end                     
 from orders; 
 
-- calculate active months for each order for each time window
-- a month = 30 days
drop table if exists user_act_month_1; 
create temporary table user_act_month_1 as
 select *,
        -- short_term
        case when datediff(d,st_start,st_end) < 0 
             then null
             else (datediff(d,st_start,st_end)+1.0)/30.0 end as st_act_mo,
        -- mid-term
        case when datediff(d,mt_start,mt_end) < 0 
             then null
             else (datediff(d,mt_start,mt_end)+1.0)/30.0 end as mt_act_mo,
        -- long-term     
        case when datediff(d,lt_start,lt_end) < 0 
             then null
             else (datediff(d,lt_start,lt_end)+1.0)/30.0 end as lt_act_mo,
        -- all_time     
        case when datediff(d,all_time_start,all_time_end) < 0 
             then null
             else (datediff(d,all_time_start,all_time_end)+1.0)/30.0 end as all_time_act_mo,
        -- last year     
        case when datediff(d,ly_start,ly_end) < 0 
             then null
             else (datediff(d,ly_start,ly_end)+1.0)/30.0 end as ly_act_mo
   from user_act_month_0;  
 
-- group by user_id to calculate total active months for each time window
-- (some users may have placed several orders)
drop table if exists user_act_month_2;
create temporary table user_act_month_2 as
select src_fan_id, 
...
drop table if exists user_act_month;
create temporary table user_act_month as
select src_fan_id,
       case when st_act_mo > 3 then 3 else st_act_mo end as st_act_mo_clean,
       case when mt_act_mo > 9 then 9 else mt_act_mo end as mt_act_mo_clean,
       case when lt_act_mo > (max_months_available - 12) then (max_months_available - 12) else lt_act_mo end as lt_act_mo_clean,
       case when all_time_act_mo > max_months_available then max_months_available else all_time_act_mo end as all_time_act_mo_clean,
       case when ly_act_mo > 12 then 12 else ly_act_mo end as ly_act_mo_clean
from user_act_month_2; 
