...
drop table if exists duration_ly;
create temporary table duration_ly as
select 
       src_fan_id,
  ...
  from (select distinct src_fan_id,production_id,content_duration,content_class 
          from al_t3_vc_available_content
         where network_first_stream >= dateadd(mon,-12,today_date)        -- last year cutoff
          and network_first_stream <  today_date                          -- last year cutoff
        )  -- short-term cutoff
 group by src_fan_id;


drop table if exists al_t3_vc_content_duration_ly;
create table al_t3_vc_content_duration_ly as select * from duration_ly;
grant select on al_t3_vc_content_duration_ly to public;
select top 1000* from duration_ly order by random();
select top 1000* from duration_ly order by random();

select top 1000* from al_t3_vc_user_consumption_ly order by random();
select count(*) from al_t3_vc_user_consumption_ly; --3,335,125
select count(*) from al_t3_vc_user_consumption_ly where act_months is null;--509283, 15% of apple users are missing??????