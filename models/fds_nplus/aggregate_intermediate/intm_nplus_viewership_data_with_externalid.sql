...
select  src_fan_id, stream_id, stream_start_dttm as min_time, stream_end_dttm as max_time, time_spent, external_id, stream_type_cd, 
        ...
        from {{source('fds_nplus','fact_daily_content_viewership')}}
         where external_id in (select network_id from {{ref('intm_nplus_event_lite_log_data')}})
        and trunc(live_start_schedule) in (select premiere_date from {{ref('intm_nplus_event_lite_log_data')}})