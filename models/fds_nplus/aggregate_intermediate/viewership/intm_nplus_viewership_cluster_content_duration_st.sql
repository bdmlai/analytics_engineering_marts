/***********************************************************************************/
/*        4.  TIER 3 MODEL - CONTENT DURATION CALCULATION(ST)                      */
/* DB:ANALYTICS_WORKSPACE SCHEMA:CONTENT                                           */
/* INPUT TABLES: intm_nplus_viewership_cluster_available_content                   */
/* OUPUT TABLES: intm_nplus_viewership_cluster_content_duration_st                  */
/***********************************************************************************/
{{
    config(
        materialized='table',
...
base as (
    ...
    where network_first_stream >= dateadd(mon,-3,as_on_date) and network_first_stream <= as_on_date
...
duration_ly as (
    ...
    select
        ...
        src_fan_id,
        'st' as time_window,                   -- time window indicator
