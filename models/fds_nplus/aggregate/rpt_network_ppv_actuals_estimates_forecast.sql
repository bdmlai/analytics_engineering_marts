...
"drop table if exists #final_view_n;
create table  #final_view_n as 
(Select  adds_date,	
	hour as adds_time,
	...
	count(distinct case when flag = 'Trial' or flag = 'NA' then customerid end) as  trial_adds,(paid_adds+trial_adds)  total_adds from
		...
		trunc(convert_timezone('AMERICA/NEW_YORK', sysdate-1)) else trunc(convert_timezone('AMERICA/NEW_YORK', sysdate)) end ) as adds_date,	
		   ...
		   and payload_data_payment_provider not in ('ROKU_IAP','GOOGLE_IAP')
...
union all
        ...
        inner join (select * from #final_view_n
	) as b
...
and UPPER(payment_method) in ('MLBAM') and Upper(official_run_flag)='OFFICIAL'  --'ROKU''APPLE'
