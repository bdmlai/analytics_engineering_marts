 {{
  config({
	"materialized": 'view',
	"post-hook": ["COMMENT ON COLUMN fds_nplus.vw_aggr_nplus_PPV_Week_Adds_Tracking.date IS 'Date and timestamp of the scheduled live events as received from LES';
					...
					COMMENT ON COLUMN fds_nplus.vw_aggr_nplus_PPV_Week_Adds_Tracking.start_date IS 'date of Event Date minus 6 days';
					COMMENT ON COLUMN fds_nplus.vw_aggr_nplus_PPV_Week_Adds_Tracking.end_date IS 'date of Event date plus 2 days';  
					...
					COMMENT ON COLUMN fds_nplus.vw_aggr_nplus_PPV_Week_Adds_Tracking.paid_winbacks IS 'Roll up of all paid winback subscriptions for the day';  
					COMMENT ON COLUMN fds_nplus.vw_aggr_nplus_PPV_Week_Adds_Tracking.new_paid IS 'Roll up of all paid subscriptions who have paid for the first time';  
					COMMENT ON COLUMN fds_nplus.vw_aggr_nplus_PPV_Week_Adds_Tracking.free_trial_subs IS 'Total trial adds for the day';
...
FROM
  ...
  LEFT JOIN 
	...
	from {{ref('aggr_daily_subscription')}}
	where
...
RIGHT JOIN
	(
                 ...
                 from {{ref('aggr_nplus_daily_forcast_output')}}
                 where forecast_date=(select max(forecast_date) from {{ref('aggr_nplus_daily_forcast_output')}})
