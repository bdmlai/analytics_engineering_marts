...
COMMENT ON TABLE fds_yt.rpt_yt_daily_wwe_video
IS
    'Report table consists of total views  at video_id,region and talent  level on daily basis';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.report_date
IS
     'Report Date';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.video_id
IS
     'The ID of a YouTube video. In the YouTube Data API, this is the value of a video resources idproperty.';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.region_code
IS
...
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.total_views
IS
...
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.talent
IS
     'List of talents';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.etl_batch_id
IS
    'Unique ID of DBT Job used to insert the record';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.etl_insert_user_id
IS
    'Unique ID of the DBT user that was used to insert the record';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.etl_insert_rec_dttm
IS
    'Date Time information on when the DBT inserted the record';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.etl_update_user_id
IS
    'Unique ID of the DBT user which was used to update the record manually';
COMMENT ON COLUMN fds_yt.rpt_yt_daily_wwe_video.etl_update_rec_dttm
IS
