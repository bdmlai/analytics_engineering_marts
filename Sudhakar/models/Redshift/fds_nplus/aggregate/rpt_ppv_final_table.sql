...
where update_date=(select max(update_date) from {{source('udl_nplus','raw_da_weekly_ppv_hourly_comps_new')}})
and as_on_date=(select max(as_on_date) from {{source('udl_nplus','raw_da_weekly_ppv_hourly_comps_new')}})
and exists (select 1 from {{source('udl_nplus','raw_da_weekly_ppv_hourly_comps_new')}} where event_date between
...
and event_type not in ('current_ppv','comp1','comp2','comp3') --!='comp3' 
...
(select avg(ghw_sum) ghw_avg,adds_day_of_week from
(select sum(paid_adds)+sum(trial_adds) ghw_sum ,adds_day_of_week,event_name  from #final_table where event_type not in ('current_ppv','comp1','comp2','comp3')
group by adds_day_of_week,event_name) group by 2
)),
...
#t3_comps_ghw_hour_moving_pct as 
