...
left join ( select distinct iso_alpha2_ctry_cd,region_nm from {{source('cdm','dim_region_country')}} where ent_map_nm = 'GM Region'  ) f
on a.country_cd = f.iso_alpha2_ctry_cd 
left join ( select  iso_alpha2_ctry_cd,src_country_name as country_nm from {{source('cdm','dim_country')}} where src_sys_Cd='iso' ) k
