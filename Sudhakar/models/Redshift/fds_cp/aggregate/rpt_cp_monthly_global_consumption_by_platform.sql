...
from 
	...
	from {{source('fds_yt','rpt_yt_wwe_engagement_daily')}} where month = date_trunc('month',current_date-25)
	group by 1,2,3
	union all 
	select  'UGC' as type, 
			date_trunc('month',to_date(report_date,'yyyymmdd')) as month, 
			...
			sum(views) as views,
			SUM(watch_time_minutes) AS view_time_minutes
	from {{source('fds_yt','rpt_yt_ugc_engagement_daily')}} where month = date_trunc('month',current_date-25)
	group by 1,2,3) a
left join
	{{source('cdm','dim_region_country')}} c
...
from
		(select date, 
				...
				minutes_watched, 
				live_views  
		from {{source('hive_udl_cp','twitch_monthly_stream_sessions')}}
...
from 
	(select platform, 
			...
			views, 
			LAG(views)  over (partition by platform, type, Country order by month) as prev_month_views
	from #all_data
	group by platform, month, type, Country, views) as a
left join 
	(select platform, 
			month, 
			type, 
			...
			hours_watched, 
			LAG(hours_watched)  over (partition by platform, type, Country order by month) as prev_month_hours
	from #all_data
	group by platform, month, type, Country, hours_watched) as b
