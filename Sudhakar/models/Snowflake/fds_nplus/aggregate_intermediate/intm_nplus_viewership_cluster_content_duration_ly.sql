/***********************************************************************************/
/*        4.  TIER 3 MODEL - CONTENT DURATION CALCULATION(LY)                      */
/* DB:ANALYTICS_WORKSPACE SCHEMA:CONTENT                                           */
/* INPUT TABLES: intm_nplus_viewership_cluster_available_content                   */
/* OUPUT TABLES: intm_nplus_viewership_cluster_content_duration_ly                 */
/***********************************************************************************/
{{
    config(
        materialized='table',
...
base as (
    ...
    where network_first_stream >= dateadd(mon,-12,as_on_date) and network_first_stream <= as_on_date
...
duration_ly as (
    ...
    select
        ...
        src_fan_id,
        'ly' as time_window,                   -- time window indicator
