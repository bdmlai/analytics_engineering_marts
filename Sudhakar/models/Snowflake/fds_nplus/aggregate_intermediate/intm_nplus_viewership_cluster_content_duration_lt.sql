/***********************************************************************************/
/*        4.  TIER 3 MODEL - CONTENT DURATION CALCULATION(LT)                      */
/* DB:ANALYTICS_WORKSPACE SCHEMA:CONTENT                                           */
/* INPUT TABLES: intm_nplus_viewership_cluster_available_content                   */
/* OUPUT TABLES: intm_nplus_viewership_cluster_content_duration_lt                 */
/***********************************************************************************/
{{
    config(
        materialized='table',
...
base as (
    ...
    where network_first_stream >= '2015-01-01 00:00:00' and network_first_stream <= dateadd(mon,-12,as_on_date)
...
duration_ly as (
    ...
    select
        ...
        src_fan_id,
        'lt' as time_window,                   -- time window indicator
